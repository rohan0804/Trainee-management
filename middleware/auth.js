...
exports.auth = async (req,res,next)=>{
    try {
        ...
        jwt.verify(refreshToken,config.get('refreshTokenSecret'),async (error,decode)=>{
            if(error){
            return res.redirect('/');
            }
            else{
                const currentTime = parseInt((Date.now()/1000).toFixed(0));
                jwt.verify(accessToken,config.get('jwtSecret'),async (err,accessDecode)=>{
                if(err || accessDecode.exp-currentTime<=10*60){
                    ...
                    const user = await Auth.findByPk(decode.auth_id);
                    ...
                    const auth = await Auth.findByPk(accessTokenDecode.auth_id);
